    ...
    <div className="space-y-4 pb-24 max-w-4xl mx-auto px-4 py-3">
      ...
      <div className="rounded-2xl bg-gradient-to-b from-surface to-canvas border border-line p-4 sm:p-5 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-brand/10 text-brand-soft border border-brand/20 font-bold">
                Controle Geral
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-4 pt-4 border-t border-line">
          <div className="bg-canvas/70 p-3 rounded-xl border border-line/80">
            <span className="text-[10px] text-ink-muted font-mono uppercase">Pilotos Conectados:</span>
            <p className="text-lg font-black text-ink font-mono">1.284</p>
          </div>
          <div className="bg-canvas/70 p-3 rounded-xl border border-line/80">
          ...
          <div className="bg-canvas/70 p-3 rounded-xl border border-line/80">
            ...
            <p className="text-lg font-black text-brand-soft font-mono">98.2%</p>
          </div>
          <div className="bg-canvas/70 p-3 rounded-xl border border-line/80">
      ...
      <div className="rounded-2xl bg-surface/80 border border-line p-4 shadow-md">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] text-ink-muted font-mono">LGPD Compliance Ativo</span>
        </div>
        ...
        <div className="space-y-2.5">
          {shops.map((shop) => (
            <div
              key={shop.id}
              className="bg-canvas/80 border border-line rounded-xl p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {shop.isVerified ? (
                    ...
                    <span className="text-[10px] px-1.5 py-0.2 rounded bg-brand/15 text-brand-soft font-medium">
                ...
                <div className="flex gap-1.5 mt-1.5">
                  {(shop.specialties ?? []).map((spec, i) => (
                    <span key={i} className="text-[10px] px-1.5 py-0.5 bg-surface text-ink-muted rounded font-mono">
                      {spec}
                    </span>
                  ))}
                </div>
      ...
      <div className="rounded-2xl bg-surface/80 border border-line p-4 shadow-md">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-ink-faint font-mono">Autenticação Ativa</span>
        </div>
        ...
        <div className="space-y-2">
          {storageService.getUsers().map((u) => (
            <div
              key={u.id}
              className="bg-canvas/80 border border-line/80 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
            >
              ...
              <div className="text-[10px] text-ink-faint font-mono self-end sm:self-center">
      ...
      <div className="rounded-2xl bg-surface/80 border border-line p-4 shadow-md">
        ...
        <div className="space-y-2">
          {sosAlerts.map((alert) => (
            <div
              key={alert.id}
              className="bg-canvas/80 border border-line rounded-xl p-3 flex items-center justify-between text-xs"
            >
              <div>
                <span className="font-bold text-ink">{alert.petitionerName}</span>
                <span className="text-ink-muted ml-2">({alert.locationReference})</span>
                <p className="text-[11px] text-ink-muted mt-0.5">{alert.details}</p>
              </div>

              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-elevated text-ink-muted">
                  {alert.status}
                </span>
